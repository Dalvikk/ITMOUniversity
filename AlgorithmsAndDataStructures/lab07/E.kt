...
fun main() {
    val n = readInt()
    val array = Array(n) { readLong() }
    val tree = IntervalTreeE(array, Long::plus)
    ...
    output {
        for (line in lines) {
            ...
            if (cmd == "sum") {
                ...
                tree.update(a.toInt(), b.toLong())
            }
        }
    }
}

class IntervalTreeE<T>(private val array: Array<T>, private val f: (T, T) -> T) {
