...
public class MapChecker<M> {
    ...
    /* package-private*/ void check(final Settings settings) {
        ...
        final SetHistory<Integer> keys = new SetHistory<>(() -> test.randomInt(-settings.range, settings.range), test.getRandom());
        final SetHistory<Value> values = new SetHistory<>(() -> Value.string(test.randomString(Randomized.ENGLISH)), test.getRandom());
        ...
        for (int i = 0; i < settings.updates; i++) {
            settings.tick(i);
            test.randomItem(updaters).accept(state);
            check(state);
        }
    }
