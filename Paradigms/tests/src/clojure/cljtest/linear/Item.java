...
public interface Item {
    ...
    static Stream<Item> args(final int argc, final Item shape, final Random random) {
    ...
    final class Value implements Item {
        ...
        @Override
        public Value refill(final Random random) {
            return new Value((random.nextInt(99) + 1) / 10.0);
        }

        @Override
        public Engine.Result<?> toClojure() {
            return LinearTest.number(value);
        }

        @Override
        public boolean equals(final Object obj) {
            return obj instanceof Value && Asserts.isEqual(1e-7, value, ((Value) obj).value);
        }
    ...
    final class Vector implements Item {
        ...
        @Override
        public Vector refill(final Random random) {
            return vector(items.stream().map(item -> item.refill(random)));
        }

        @Override
        public Engine.Result<?> toClojure() {
            return LinearTest.vector(toArray(items.stream().map(Item::toClojure)));
        }
...
//            System.err.println("Testing? " + result.context);
        ...
        private void nextTest(final TestCounter counter) {
            counter.nextTest();
            if (counter.getTest() % 1000 == 0) {
                System.out.println("Test " + counter.getTest());
            }
        }

        public void test(final int args, final Item shape, final TestCounter counter, final Random random) {
            test(Collections.nCopies(args, shape), counter, random);
        }

        public void test(final List<Item> shapes, final TestCounter counter, final Random random) {
