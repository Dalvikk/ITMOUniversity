...
public class LinearTest extends BaseTest {
    ...
    protected void expectException(final List<Item.Fun> funs, final int[] okDims, final int[][] failDims) {
        ...
        Stream.concat(Arrays.stream(failDims), corrupted(okDims)).map(Item::generator).forEach(fail -> {
            expectException(counter, funs, ok, fail);
            expectException(counter, funs, fail, ok);
        });
    }
    ...
    @SafeVarargs
    protected static void expectException(final TestCounter counter, final List<Item.Fun> funs, final Supplier<Item>... generators) {
        for (final Item.Fun fun : funs) {
            // TODO
            fun.expectException(counter, Arrays.stream(generators).map(Supplier::get));
        }
    }
    ...
    protected void test(final int args) {
        ...
        for (int complexity = 1; complexity <= 20; complexity++) {
            for (int dim1 = 1; dim1 <= complexity; dim1++) {
                ...
                if (dim2 > 0 || hard) {
                    ...
                    M_BY_V.test(counter, Stream.of(generator.get().refill(random), Item.generator(dim2).get().refill(random)));
            ...
            M_BY_M.test(counter, IntStream.range(0, args).mapToObj(i -> Item.generator(dims[i], dims[i + 1]).get().refill(random)));
        }
